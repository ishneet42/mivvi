...
export async function createGroup(groupFormValues: GroupFormValues, ownerId?: string) {
  ...
  // Seed the OWNER as a GroupMember too so membership checks succeed.
  if (ownerId) {
...
export async function getGroups(groupIds: string[], userId?: string) {
  return (
    await prisma.group.findMany({
      where: {
        id: { in: groupIds },
        // If a user is provided, only return groups they're a member of.
        ...(userId ? { members: { some: { clerkUserId: userId } } } : {}),
      },
...
// All groups the signed-in user belongs to (any role), for the /groups dashboard.
  ...
  return (
    await prisma.group.findMany({
      ...
      include: {
        ...
        members: { where: { clerkUserId: userId }, select: { role: true } },
      },
    ...
    currentUserRole: g.members[0]?.role ?? 'MEMBER',
  }))
}
...
export async function updateGroup(
  ...
  return prisma.group.update({
    ...
    data: {
      ...
      currencyCode: groupFormValues.currencyCode,
      participants: {
        deleteMany: existingGroup.participants
